...
type UploadReviewImageBody = {
  file_base64?: string
  content_type?: string
}

export const POST = async (req: MedusaRequest<UploadReviewImageBody>, res: MedusaResponse) => {
  const body = req.body ?? {}
  ...
  const fileName = `${storeId}-review-${randomBytes(6).toString("hex")}.${ext}`
  const uploadDir = resolveStoreMediaUploadDir("review")
  await mkdir(uploadDir, { recursive: true })
  await writeFile(path.join(uploadDir, fileName), validation.buffer)

  const imageUrl = buildStoreMediaPublicUrl("review", fileName, resolveStoreMediaBaseUrl(req))
