...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    if (message === "S2B_PRINT_FILE_REQUIRED") {
      return sendError(
        res,
        400,
        "VALIDATION_ERROR",
        "Product has no print file. Regenerate from AI Studio before opening S2BDIY editor."
      )
    }
    if (message === "S2B_BLANK_PRODUCT_CONFIG_REQUIRED") {
      return sendError(
        res,
        400,
        "VALIDATION_ERROR",
        "Missing S2BDIY blank product configuration (basic product, size, color)."
      )
    }
    console.error("[admin/design-config] failed:", message)
