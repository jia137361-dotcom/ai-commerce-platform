...
// 导入函数
// =============================================================================
export default async function importS2bdiyCatalog({ container }: ExecArgs) {
  ...
  for (const cat of level1Categories) {
    try {
      await storeCoreService.createProductCategories({
        ...
        parent_id: null,
        status: "active",
        raw_json: { level: 1, product_count: cat.product_count, source: "s2bdiy" }
      })
    } catch (e) {
  ...
  for (const cat of level2Categories) {
    try {
      await storeCoreService.createProductCategories({
        ...
        status: "active",
        raw_json: { level: 2, product_count: cat.product_count, source: "s2bdiy" }
      })
    } catch (e) {
      console.log(`  跳过已存在的分类: ${cat.name}`)
    }
  ...
  for (const cat of level3Categories) {
    try {
      await storeCoreService.createProductCategories({
        ...
        parent_id: cat.parent_id,
        status: "active",
        raw_json: { level: 3, product_count: cat.product_count, source: "s2bdiy" }
      })
    } catch (e) {
      console.log(`  跳过已存在的分类: ${cat.name}`)
    }
  ...
  console.log("")
  console.log("下一步：")
  console.log("1. 通过 S2BDIY API 批量同步产品数据")
  console.log("2. 运行: npx medusa exec ./src/scripts/sync-s2bdiy-products.ts")
}
