...
const createBuyerSession = async (email) => {
  ...
  if (registered) {
    const created = await jsonRequest("/store/customers", { method: "POST", token, body: { email, first_name: "Buyer", last_name: email.includes("buyer_a") ? "A" : "B", phone: "+15555550123" } })
  ...
  console.log(`BUYER_SESSION_COOKIE=${email}:${cookie?.split("=", 1)[0] ?? "missing"}`)
  if (!customerId) {
...
const createBuyerCart = async (buyer, variantId) => {
  const created = await jsonRequest("/store/carts", { method: "POST", cookie: buyer.cookie, token: buyer.token, body: { currency_code: "usd" } })
  const cartId = created.payload.cart_id || created.payload.id
  await jsonRequest(`/store/carts/${cartId}/line-items`, { method: "POST", cookie: buyer.cookie, token: buyer.token, body: { variant_id: variantId, quantity: 1 } })
  await jsonRequest(`/store/carts/${cartId}/customer`, { method: "POST", cookie: buyer.cookie, token: buyer.token, body: {} })
  const cart = (await jsonRequest(`/store/carts/${cartId}`, { cookie: buyer.cookie, token: buyer.token })).payload
  return { cartId, cart }
}

const sellerAuth = await jsonRequest("/auth/user/emailpass", { method: "POST", admin: true, body: { email: SELLER_EMAIL, password: PASSWORD } })
const sellerProducts = await jsonRequest("/admin/store-products?status=published&limit=20", { token: sellerAuth.payload.token, admin: true })
...
if (!buyerProduct) throw new Error("Buyer product grid did not return the isolated product")
...
if (cartA.cart.items?.[0]?.variant_id !== variantId || cartB.cart.items?.[0]?.variant_id !== variantId) throw new Error("Cart line variant mismatch")
if (!(cartTotal(cartA.cart) > 0) || !(cartTotal(cartB.cart) > 0)) throw new Error("Cart total is not positive")

await jsonRequest(`/store/carts/${cartA.cartId}/contact`, { method: "PUT", cookie: buyerA.cookie, token: buyerA.token, body: { email: BUYER_A_EMAIL, phone: "+15555550123" } })
const region = await jsonRequest(`/store/regions/${encodeURIComponent(cartA.cart.region_id)}`, { cookie: buyerA.cookie, token: buyerA.token })
const countryCode = region.payload.region?.countries?.[0]?.iso_2 || region.payload.region?.countries?.[0]?.iso2 || "us"
await jsonRequest(`/store/carts/${cartA.cartId}/address`, { method: "PUT", cookie: buyerA.cookie, token: buyerA.token, body: { email: BUYER_A_EMAIL, phone: "+15555550123", shipping_address: { first_name: "Buyer", last_name: "A", address_1: "525 Market Street", city: "Test City", province: "Test Province", postal_code: "100000", country_code: countryCode } } })
const shipping = await jsonRequest(`/store/carts/${cartA.cartId}/shipping-options`, { cookie: buyerA.cookie, token: buyerA.token })
...
await jsonRequest(`/store/carts/${cartA.cartId}/shipping-methods`, { method: "POST", cookie: buyerA.cookie, token: buyerA.token, body: { option_id: pricedShipping.id } })

const regionId = cartA.cart.region_id
const providers = regionId ? await jsonRequest(`/store/payment-providers?region_id=${encodeURIComponent(regionId)}`, { cookie: buyerA.cookie, token: buyerA.token }) : { payload: { payment_providers: [] } }
const stripeProvider = providers.payload.payment_providers?.find((provider) => provider.id === "pp_stripe_stripe")
if (!stripeProvider) throw new Error("Card Stripe provider pp_stripe_stripe is unavailable")
...
console.log(JSON.stringify({
  ...
  buyerA: {
    ...
    shippingOptions: shipping.payload.shipping_options?.map((option) => ({ id: option.id, amount: option.amount, currency_code: option.currency_code })) ?? [],
  ...
  stripeProviderAvailable: Boolean(stripeProvider),
}, null, 2))
