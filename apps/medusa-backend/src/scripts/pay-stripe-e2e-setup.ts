...
const storeId = process.env.PAY_STRIPE_TEST_STORE_ID?.trim() || STORE_ID
const productId = process.env.PAY_STRIPE_TEST_PRODUCT_ID?.trim() || PRODUCT_ID
const sellerEmail = process.env.PAY_STRIPE_TEST_SELLER_EMAIL?.trim().toLowerCase() || SELLER_EMAIL
const allowRuntimeAccountCreate = process.env.PAY_STRIPE_CREATE_RUNTIME_ACCOUNTS === "true"
...
const assertSafeRuntimeSetup = () => {
  if (process.env.PAY_STRIPE_E2E_SETUP !== "true") {
    throw new Error("Set PAY_STRIPE_E2E_SETUP=true to create the isolated PAY-STRIPE fixture")
  }
  if (process.env.NODE_ENV !== "development") {
    throw new Error("Stripe E2E fixture setup only runs with NODE_ENV=development")
  }
  const dbUrl = process.env.DATABASE_URL ?? ""
  if (/prod|production/i.test(dbUrl) || !/localhost|127\.0\.0\.1|5433|ai_commerce/i.test(dbUrl)) {
    throw new Error("Refusing Stripe E2E fixture setup because DATABASE_URL does not look local/development")
  }
}
...
async function createRuntimeSellerIfMissing(container: ExecArgs["container"]) {
  ...
  if (!password || password.length < 8) {
    throw new Error("PAY_STRIPE_TEST_PASSWORD is required to create the isolated Stripe runtime seller")
  }
  ...
  const user = await userModule.createUsers({
    ...
    metadata: {
      test_only: true,
      fixture: "stripe-payment-closure",
      created_by: "pay-stripe:e2e:setup",
    },
  })
  ...
  if (!registered.success || !registered.authIdentity?.id) {
    throw new Error(registered.error ?? "Unable to create isolated Stripe runtime seller auth identity")
  }
  ...
  if (!verified.success) {
    throw new Error("Unable to verify isolated Stripe runtime seller credentials")
  }
  console.log("PAY_STRIPE_RUNTIME_SELLER_CREATED=true")
...
export default async function setupPayStripeE2e({ container }: ExecArgs) {
  ...
  if (!existingStore) {
    await storeCore.createStores({
      ...
      name: `Stripe Runtime Test Store ${storeId}`,
  ...
  const settingsData = {
    store_id: storeId,
    brand_name: `Stripe Runtime Test Store ${storeId}`,
    logo_url: "https://placehold.co/256x256/png?text=MKT01",
    support_email: sellerEmail,
    seo_title: "Stripe Runtime Test Store",
  ...
  if (!product) {
    product = await storeCore.createProducts({
      id: productId,
      store_id: storeId,
      title: `Stripe Runtime Test Product ${productId}`,
  ...
  console.log(`PAY_STRIPE_TEST_SELLER=${sellerEmail}`)
  console.log(`PAY_STRIPE_TEST_STORE=${storeId}`)
  console.log(`PAY_STRIPE_TEST_PRODUCT=${productId}`)
