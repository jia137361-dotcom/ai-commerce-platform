...
export function ShopBrowseControls({ categories, activeCategoryId, onCategoryChange, query, onQueryChange, sort, onSortChange }: ShopBrowseControlsProps) {
  return (
    <section className="buyer-shop-controls" aria-label="Browse products">
      <nav className="buyer-shop-category-tabs" aria-label="Product categories">
        {categories.map((category) => <button className={activeCategoryId === category.id ? "active" : ""} key={category.id} type="button" onClick={() => onCategoryChange(category.id)}>{category.name}</button>)}
      </nav>
