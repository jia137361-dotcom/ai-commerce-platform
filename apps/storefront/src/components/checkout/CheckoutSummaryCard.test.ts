...
describe("CheckoutSummaryCard", () => {
  it("renders real cart item and totals", () => {
    const html = renderToStaticMarkup(createElement(CheckoutSummaryCard, { cart, canPlaceOrder: true, disabledReason: "", onPlaceOrder: () => undefined, placing: false }))
    expect(html).toContain("Real cart item")
    expect(html).toContain("$21.25")
    expect(html).toContain("Place order")
  })
  it("disables place order for invalid checkout state", () => expect(renderToStaticMarkup(createElement(CheckoutSummaryCard, { cart, canPlaceOrder: false, disabledReason: "Missing price", onPlaceOrder: () => undefined, placing: false }))).toContain("disabled"))
})
